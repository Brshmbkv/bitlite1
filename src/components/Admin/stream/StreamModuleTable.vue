    ...
    >
      ...
      <template #body="{items}">
        ...
        >
          ...
          <td>
            <div
              class="awesome-switch"
              @click.stop.prevent="changeModuleActive(item)"
            >
...
export default {
  ...
  data() {
    return {
      ...
      headers: [
        ...
        { text: 'Править', value: 'options', align: 'left', sortable: false },
      ],
    }
  },
  ...
  methods: {
    async changeModuleActive(item) {
      console.log(item)
      await this.$axios
        .post(`admin/streams/${item.group_id}/changeGroupModuleActive`, {
          id: item.id,
          value: !item.active,
        })
        .then((res) => {
    ...
    async changeModuleStartsAt(date, item) {
      await this.$axios
        .post('admin/streams/changeGroupModuleStartsAt', {
          id: item.id,
          starts_at: date,
        })
        .then((res) => {
    ...
    async removeModule(id) {
      await this.$axios
        .post(`admin/streams/removeModule`, {
          course_module_id: id,
