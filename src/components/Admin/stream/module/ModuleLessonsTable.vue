  ...
  <div class="mb-8">
    ...
    >
      ...
      <template v-slot:item.count_attendant="{ item }">
        <span  :style="{ color: item.count_attendant > 0 ? '#5CC689' : '#5A6275'}">{{ item.count_attendant }}</span> / {{ studentsCount }}
      </template>
...
export default {
  ...
  data() {
    return {
      headers: [
        ...
        { text: 'Средняя успеваемость', value: 'average_score', sortable: true },
        { text: 'Посещаемость', value: 'count_attendant', align: 'center', sortable: true },
  ...
  methods: {
    async removeEvent(id) {
      await this.$axios.delete(`admin/events/${id}`)
        .then(res => {
          if(res && res.data) {
