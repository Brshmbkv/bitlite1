<template>
  <v-row>
    ...
    <v-col cols="12" class="d-flex align-center justify-space-between">
      <h2>{{ module_title }}: вводный курс</h2>
      <v-btn
    ...
    <v-col v-if="module_id === 13" md="8" cols="12">
      <p class="chapters_table__module-description">Бесплатный модуль курса Java Standard Edition. Попробуйте программирование на Java и напишите свой первый код</p>
    </v-col>
    <v-col>
      <v-simple-table class="my-table">
        ...
        <template v-if="chapters">
          <router-link
            ...
            "
            :style="{
                color: isAvailable(ekey) && !chapter.completed
                    ? '#171729' // black
                    : (chapter.completed)
                      ? '#0BC3B8' // green
                      : '#9FA4B1', // gray
                cursor: isAvailable(ekey) ? 'pointer' : 'default'
              }"
          >
            ...
            >
              <template #activator="{ on, attrs }">
                ...
                >
                  ...
                  <span
                    v-if="!chapter.available_by_date && chapter.starts_at"
                    class="d-block black--text mt-3 error--text"
                    style="font-weight: 600;"
                  >
                    Урок будет доступен: {{ chapter.starts_at | deadlineFormat }}
                  </span>
                  <span
                    v-if="chapter.deadline && chapter.available_by_date"
                    class="d-block black--text mt-3 warning--text"
                    style="font-weight: 600;"
                  >
                    Дедлайн задач: {{ chapter.deadline | deadlineFormat }}
                  </span>
                </td>
              </template>
              <span>Доступ откроется после прохождения урока: "{{ chapters[ekey-1] && chapters[ekey - 1].title }}"</span>
            </v-tooltip>
            <td
              v-else
              class="font-weight-medium py-2"
            >
              ...
              <v-chip
                v-if="chapter.deadline && chapter.available_by_date && !chapter.completed"
              ...
              >
                ...
                <span
                  style="font-size: 12px;font-weight: 600; color: #E7AE1E"
                  class="text-uppercase"
                >
                  Дедлайн задач: {{ chapter.deadline | deadlineFormat }}
                </span>
...
export default {
  ...
  props: {
    ...
    // !available - if course is locked (starting soon)
    available: {
      type: Boolean,
      default: true,
    },
  ...
  methods: {
    ...
    isAvailable(ekey) {
      return this.chapters[ekey].event_id ?
        ((this.available &&
          (
            this.chapters[ekey-1] // chapters[-1] returns undefined
            && !this.chapters[ekey-1].next_available // prev chapter not complete
            && (this.chapters[ekey].available_by_date === null)
          ) || this.chapters[ekey].available_by_date //locked by date
        ) || !this.chapters[ekey-1])
        :
        (this.available && // FOR LITE
          (
            this.chapters[ekey-1]
            && this.chapters[ekey-1].next_available
          ) || !this.chapters[ekey-1]
        )
    },
    testsLeft(ch) {
...
.chapters_table__module-description {
  font-size: 14px;
