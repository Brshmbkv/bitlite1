<template>
  <v-container>
      ...
      <v-col cols="auto pa-0 ml-4 ">
        <div class="d-flex flex-column">
          ...
          <template v-else>
            ...
            <p
              v-if="field_nickname"
              style="font-size: 24px; line-height: 36px; font-weight: 500;"
              class="profile__info-text"
            >
              @{{ field_nickname }}
            </p>
    ...
    <div class="profile__block">
      ...
      <ul style="list-style-type: number">
        <li
          v-for="stat in stats"
          :key="stat.title"
          style="font-size: 18px; line-height: 1.15; font-weight: 500;"
        >
          <span>{{ stat.title }}: {{ stat.value }} {{ stat.value_text }}</span>
        </li>
      </ul>
      <!-- <v-row>
        <v-col
    ...
    <div class="profile__block">
      ...
      <v-row>
        <template v-if="loading">
          <v-col v-for="i in 3" :key="i" cols="12" sm="6" md="3">
            <CompletedCourseCard :loading="loading" />
          </v-col>
        </template>
        <template
          v-else-if="completed_courses && completed_courses.length === 0"
        >
        ...
        <template v-else>
          ...
          >
            <CompletedCourseCard
              ...
              :title="course.title"
              :date="
                course.group_course_module &&
                  course.group_course_module.starts_at
              "
              :loading="loading"
            />
          </v-col>
...
export default {
  ...
  mounted() {
    ...
    // this.store.commit("")
    Promise.all([this.fetchProfile(), this.fetchPayments()]).finally(
      () => (this.loading = false)
    )
  },
  methods: {
    ...
    changeInfo(user) {
      ...
      this.error = false
      const { first_name, last_name, nickname, email, phone } = user
