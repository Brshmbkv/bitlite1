<template>
  <v-container fluid>
    <p class="font-weight-bold" style="color: #171729;font-size: 24px">
      Добавление урока
    </p>
    <v-row>
      <v-col cols="12" md="5">
        ...
        <v-text-field
          ...
          color="#4376FB"
          :error-messages="
            addLessonErrors.hasOwnProperty('title') ? addLessonErrors.title : ''
          "
      ...
      <v-col cols="6" md="2">
        ...
        <v-text-field
          ...
          type="number"
          :error-messages="
            addLessonErrors.hasOwnProperty('order') ? addLessonErrors.order : ''
          "
          dense
          class="add-dialog__v-text-field"
        >
      ...
      <v-col v-if="lessonType !== 2" cols="12" sm="10">
        <p class="add-dialog__label-for-input" style="font-weight: 600;">
          Контент лекции
        </p>
        <LectureConstructor :value="currentContent" @save="addLessonContent" />
      </v-col>
    </v-row>
  </v-container>
</template>
...
export default {
    ...
    return {
      ...
      init: {
        images_upload_handler: function(blobInfo, success, failure) {
          ...
          this.$axios
            .post(
              'https://app.bitlab.academy/api/v1/lessons-image-upload',
              formData
            )
            .then((res) => {
              if (res && res.data) {
  ...
  methods: {
    ...
    async fetchLessonCreate() {
      await this.$axios
        .get('admin/chapters/' + this.chapter_id + '/lessons/create')
        .then((res) => {
          if (res && res.data) {
